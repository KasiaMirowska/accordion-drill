...
it('renders li empty given section props', () => {
...
it( 'renders a clicked section', () => {
    const wrapper = shallow(<Accordion section={tabSections} />);
    wrapper.find('button').at(1).simulate('click');
    expect(toJson(wrapper)).toMatchSnapshot();
})

it( 'renders only the last clicked when multiple sections are clicked', () => {
    const wrapper = shallow(<Accordion section={tabSections} />);
